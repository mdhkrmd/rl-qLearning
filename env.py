...
class Environment(tk.Tk, object):
    def __init__(self):
        ...
        self.title('Q-learning. Modification from Sichkar Valentyn')
    ...
    def build_environment(self):
        ...
        # Kolom 0
        self.obstacle1 = self.canvas_widget.create_image(0, pixels * 5, anchor='nw', image=self.obstacle2_object)
        # Obstacle 2
        self.obstacle2 = self.canvas_widget.create_image(0, pixels * 6, anchor='nw', image=self.obstacle6_object)
        # Obstacle 3
        self.obstacle3 = self.canvas_widget.create_image(0, pixels * 7, anchor='nw', image=self.obstacle5_object)
        # Obstacle 4
        self.obstacle4 = self.canvas_widget.create_image(0, pixels * 8,  anchor='nw', image=self.obstacle2_object)
        # Obstacle 5
        self.obstacle5 = self.canvas_widget.create_image(0, pixels * 9, anchor='nw', image=self.obstacle12_object)

        # Kolom 1
        # Obstacle 6
        # self.obstacle6 = self.canvas_widget.create_image(pixels,  0, anchor='nw', image=self.obstacle7_object)
        # Obstacle 7
        # self.obstacle7 = self.canvas_widget.create_image(pixels, pixels * 2, anchor='nw', image=self.obstacle9_object)
        # Obstacle 8
        self.obstacle8 = self.canvas_widget.create_image(pixels, pixels * 3, anchor='nw', image=self.obstacle10_object)
        # Obstacle 9
        self.obstacle9 = self.canvas_widget.create_image(pixels, pixels * 9, anchor='nw', image=self.obstacle4_object)
        
        # Kolom 2
        # Obstacle 10
        self.obstacle10 = self.canvas_widget.create_image(pixels * 2, pixels * 3, anchor='nw', image=self.obstacle4_object)
        # Obstacle 11
        self.obstacle11 = self.canvas_widget.create_image(pixels * 2, pixels * 4, anchor='nw', image=self.obstacle4_object)
        # Obstacle 12
        self.obstacle12 = self.canvas_widget.create_image(pixels * 2, pixels * 6, anchor='nw', image=self.obstacle4_object)
        # Obstacle 13
        self.obstacle13 = self.canvas_widget.create_image(pixels * 2, pixels * 7, anchor='nw', image=self.obstacle3_object)
        # Obstacle 14
        self.obstacle14 = self.canvas_widget.create_image(pixels * 2, pixels * 9, anchor='nw', image=self.obstacle8_object)
        # Obstacle 15
        self.obstacle15 = self.canvas_widget.create_image(pixels * 2, 0, anchor='nw', image=self.obstacle1_object)
        # Obstacle 16
        self.obstacle16 = self.canvas_widget.create_image(pixels * 2, pixels, anchor='nw', image=self.obstacle3_object)
        # Obstacle 17
        self.obstacle17 = self.canvas_widget.create_image(pixels * 2, pixels * 2, anchor='nw', image=self.obstacle4_object)
       
        # Kolom 3
        # Obstacle 18
        self.obstacle18 = self.canvas_widget.create_image(pixels * 3, pixels * 3, anchor='nw', image=self.obstacle11_object)
        # Obstacle 19
        self.obstacle19 = self.canvas_widget.create_image(pixels * 3, pixels * 9, anchor='nw', image=self.obstacle8_object)
        
        # Kolom 4
        # Obstacle 20
        self.obstacle20 = self.canvas_widget.create_image(pixels * 4, pixels * 5, anchor='nw', image=self.obstacle4_object)
        # Obstacle 21
        self.obstacle21 = self.canvas_widget.create_image(pixels * 4, pixels * 7, anchor='nw', image=self.obstacle4_object)
        # Obstacle 22
        self.obstacle22 = self.canvas_widget.create_image(pixels * 4, pixels * 9, anchor='nw', image=self.obstacle2_object)

        # Kolom 5
        # Obstacle 23
        self.obstacle23 = self.canvas_widget.create_image(pixels * 5, pixels, anchor='nw', image=self.obstacle1_object)
        # Obstacle 24
        self.obstacle24 = self.canvas_widget.create_image(pixels * 5, pixels * 3, anchor='nw', image=self.obstacle3_object)
        # Obstacle 25
        self.obstacle25 = self.canvas_widget.create_image(pixels * 5, pixels * 4, anchor='nw', image=self.obstacle4_object)
        # Obstacle 26
        self.obstacle26 = self.canvas_widget.create_image(pixels * 5, pixels * 5, anchor='nw', image=self.obstacle11_object)
        # Obstacle 27
        self.obstacle27 = self.canvas_widget.create_image(pixels * 5, pixels * 7, anchor='nw', image=self.obstacle8_object)

        # Kolom 6
        # Obstacle 28
        self.obstacle28 = self.canvas_widget.create_image(pixels * 6, pixels, anchor='nw', image=self.obstacle1_object)
        # Obstacle 29
        self.obstacle29 = self.canvas_widget.create_image(pixels * 6, pixels * 3, anchor='nw', image=self.obstacle3_object)
        # Obstacle 30
        self.obstacle30 = self.canvas_widget.create_image(pixels * 6, pixels * 7, anchor='nw', image=self.obstacle4_object)
        # Obstacle 31
        self.obstacle31 = self.canvas_widget.create_image(pixels * 6, pixels * 8, anchor='nw', image=self.obstacle11_object)

        # Kolom 7
        # Obstacle 32
        self.obstacle32 = self.canvas_widget.create_image(pixels * 7, pixels, anchor='nw', image=self.obstacle1_object)
        # Obstacle 33
        self.obstacle33 = self.canvas_widget.create_image(pixels * 7, pixels * 3, anchor='nw', image=self.obstacle3_object)
        # Obstacle 34
        self.obstacle34 = self.canvas_widget.create_image(pixels * 7, pixels * 5, anchor='nw', image=self.obstacle4_object)
        # Obstacle 35
        self.obstacle35 = self.canvas_widget.create_image(pixels * 7, pixels * 6, anchor='nw', image=self.obstacle11_object)
        # Obstacle 36
        self.obstacle36 = self.canvas_widget.create_image(pixels * 7, pixels * 7, anchor='nw', image=self.obstacle8_object)
        # Obstacle 37
        self.obstacle37 = self.canvas_widget.create_image(pixels * 7, pixels * 8, anchor='nw', image=self.obstacle8_object)

        # Kolom 8
        # Obstacle 38
        self.obstacle38 = self.canvas_widget.create_image(pixels * 8, pixels, anchor='nw', image=self.obstacle1_object)
        # Obstacle 39
        self.obstacle39 = self.canvas_widget.create_image(pixels * 8, pixels * 2, anchor='nw', image=self.obstacle3_object)
        # Obstacle 40
        self.obstacle40 = self.canvas_widget.create_image(pixels * 8, pixels * 3, anchor='nw', image=self.obstacle4_object)
        # Obstacle 41
        self.obstacle41 = self.canvas_widget.create_image(pixels * 8, pixels * 5, anchor='nw', image=self.obstacle4_object)
        # Obstacle 42
        self.obstacle42 = self.canvas_widget.create_image(pixels * 8, pixels * 8, anchor='nw', image=self.obstacle4_object)

        # Final Point
        img_flag = Image.open("images/flag.png")
        self.flag_object = ImageTk.PhotoImage(img_flag)
        self.flag = self.canvas_widget.create_image(pixels * 9, pixels * 7, anchor='nw', image=self.flag_object)

        # Uploading the image of Mobile Robot
        img_robot = Image.open("images/person_walking_40px.png")
